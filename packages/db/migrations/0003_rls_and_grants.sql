...
-- and may write that tenant's membership rows. The admin check reads the mirror
-- table, never tenant_memberships itself, so the policy cannot recurse.
...
-- API and worker: full row operations on tenant tables (RLS + policies decide rows).
GRANT SELECT, INSERT, UPDATE, DELETE ON
  securerag.tenants, securerag.principals, securerag.sessions,
  securerag.tenant_memberships, securerag.tenant_admins,
  securerag.groups, securerag.group_memberships,
  securerag.documents, securerag.document_versions, securerag.document_grants,
  securerag.chunks, securerag.jobs, securerag.retention_policies
...
-- Authorization epoch: runtime roles may bump (invoker-rights function).
GRANT SELECT, UPDATE ON securerag.authorization_epoch TO securerag_api, securerag_worker;
